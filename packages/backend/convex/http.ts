  ...
  if (metricValue === "sleep_segment") {
    ...
    return {
      ...
      value: {
        ...
        unit,
        sourceName: parseOptionalString(value.sourceName),
        sourceBundleId: parseOptionalString(value.sourceBundleId),
        timezone,
      },
    };
  }
...
function requireBearerToken(request: Request): ValidationResult<void> {
  const configuredToken = process.env.PRIVATE_API_BEARER_TOKEN;
...
export const ingestHealth = httpAction(async (ctx, request) => {
  ...
  try {
    const result = await ctx.runMutation(internal.healthInternal.upsertRawSamples, parsed.value);
...
export const getDailyHealth = httpAction(async (ctx, request) => {
  ...
  try {
    const rows = await ctx.runQuery(internal.healthInternal.listDaily, {
      from: from.value,
      to: to.value,
    });
...
export const getRawHealth = httpAction(async (ctx, request) => {
  ...
  try {
    const rows = await ctx.runQuery(internal.healthInternal.listRaw, {
      metric,
      fromMs: fromMs.value,
      toMs: toMs.value,
